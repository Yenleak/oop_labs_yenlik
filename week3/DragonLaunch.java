...
public class DragonLaunch {
    private final Vector<Device> people = new Vector<>();

    public void kidnap(Device p) {
        people.add(p);
    }
    ...
    public String line() {
        ...
        for (Device p : people) sb.append(p);
        return sb.toString();
    }
    ...
    public static DragonLaunch fromString(String s) {
        ...
        for (char ch : s.toCharArray()) {
            if (ch == 'B') dl.kidnap(new Device(Gender.BOY));
            else if (ch == 'G') dl.kidnap(new Device(Gender.GIRL));
